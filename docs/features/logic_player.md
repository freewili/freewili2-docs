...
**How to use it** — press `c`, then enter SampleRateNs SampleCount PinStart PinStop StartMode(0=now,1=rising,2=falling) TriggerPin Loop(0/1) when prompted.
...
**How to use it** — press `a`, then enter AnalogMask(bit0=Aout0 Bit3=Aout3) AnalogRateNs AnalogRes(8/16) when prompted.
...
**How to use it** — press `l`, then enter file path to load into the play buffer when prompted.
