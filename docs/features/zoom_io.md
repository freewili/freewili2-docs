---
title: "ZoomIO"
sidebar_position: 30
---
<!-- Machine-generated by the menutool product-docs generator - regenerated on every run; do not edit. -->

These commands are available three ways — in the FREE-WILi GUI app, in a serial
terminal, and on the device itself. See
[Connecting it to a computer](../start-here/connecting.md) for how to reach them.

## Stream ZoomIO Data

Enable or disable forwarding of bytes received on the ZoomIO RX FIFO to the host.

##### Examples

```
o 1   # start streaming RX data to host
o 0   # stop streaming
```

##### Notes

- Streaming only affects RX delivery to the host; it does not change ZoomIO transmit behavior or the schedule table.
- Pair with `w` (Write to FIFO), `u` (Update Schedule Table), and `p` (Setup Schedule Table) to drive traffic while observing responses.

**How to use it** — press `o`, then enter 1 to enable 0 to disable when prompted.

**What you enter** — `enable`.

## Write to FIFO

Sends a single ZoomIO message after the given delay (us).

**How to use it** — press `w`, then enter delay us and Hex Data Byte(s) Seperated By Spaces when prompted.

**What you enter** — `delay`, `data`.

## Update Schedule Table

Updates a schedule-table transmit message.

**How to use it** — press `u`, then enter Table Index, delay us and hex data byte(s) Seperated By Spaces when prompted.

**What you enter** — `table_index`, `delay`, `data`.

## Setup Schedule Table

Sets up the schedule table size (0 to disable).

**How to use it** — press `p`, then enter number of table items, 0 to disable when prompted.

**What you enter** — `number_of_entries`.

## Compile test

Compiles built-in ZoomIO milestone program and launches it on core1 as RISC-V

**How to use it** — press `c`.

## Run ZoomIO

Compile and run a ZoomIO program on the RISC-V core1

**How to use it** — press `r`, then enter the .zio script path when prompted.

**What you enter** — `path`.

## Stop ZoomIO

Reset core1 to stop the running program

**How to use it** — press `s`, then stop the running ZoomIO program when prompted.
