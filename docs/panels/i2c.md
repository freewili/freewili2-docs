...
bus is wired correctly and a device acknowledges, not to talk to a specific
part. A successful write prints a `W)` line with the address, register and
data sent. Bus clock speed and whether the internal pull-up resistors are
enabled are set from the I2C settings menu, not from this screen.
